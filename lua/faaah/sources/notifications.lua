...
local function normalize_message(msg)
  ...
  if type(msg) == "table" then
    local ok, rendered = pcall(vim.inspect, msg)
    if ok then
      return rendered
    end
  end
...
local function notify_wrapper(msg, level, opts)
  ...
  if level == vim.log.levels.ERROR then
    local normalized = normalize_message(msg)
    if should_ignore(normalized, ignore_patterns()) then
      return
    end

    log.debug("notification error: " .. normalized:sub(1, 200))
